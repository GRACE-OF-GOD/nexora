...
class EnseignantService {
  ...
  Future<List<EnseignantModel>> getEnseignants({int page = 1, int limit = 20}) async {
    try {
      ...
      return data.map((e) => EnseignantModel.fromJson(e)).toList();
    } on DioException catch (e) {
      throw _handleError(e);
    }
  }

  Future<EnseignantModel> getEnseignant(String id) async {
    try {
      final options = await _authHeaders();
      final response = await _dio.get(
        '/api/enseignants/$id',
  ...
  Future<EnseignantModel> createEnseignant(Map<String, dynamic> data) async {
    try {
      final options = await _authHeaders();
  ...
  Future<EnseignantModel> updateEnseignant(String id, Map<String, dynamic> data) async {
    try {
      final options = await _authHeaders();
  ...
  Exception _handleError(DioException e) {
    ...
    if (e.response?.statusCode == 404) {
      return Exception('Enseignant introuvable');
    }
